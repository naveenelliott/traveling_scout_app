...
function Home() {
  ...
  const handleChange = (e) => setLeague(e.target.value);
  ...
  return (
    <div className="App" style={{ textAlign: 'center', fontFamily: 'sans-serif' }}>
      <h1 className="page-title">Game Travel Map</h1>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 240px)' }}>
        <div style={{ flex: 1 }}>
          <GameMap data={data} selectedIndex={selectedIndex} setSelectedIndex={setSelectedIndex} />
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px' }}>
          <GameTable data={data} selectedIndex={selectedIndex} setSelectedIndex={setSelectedIndex} />
        </div>
      </div>
    </div>
  );
